export function createPlayerForGallery(data = null, selector = 'player') {
    ...
    if (data) {
        ...
        if(data.species) {
            ...
            species = `<span class="w-full  ml-2 text-base"> ${speciesText}</span>`
        }
    }
    let boxHtml = `
    <div class="flex flex-col items-center ${selector} min-h-full fade-in border border-blue-700 p-2 rounded">
    <div class="w-full flex flex-wrap  flex-col  items-center ">
                <div class="no-image mt-2 mr-4 h-[200px] w-[250px] overflow-hidden rounded-xl shadow-xl">
                    <img class="object-cover  w-full h-full bird-image transition duration-300 fade-in" src="${image}" alt="name">
                </div>
                <div class="flex flex-col w-full items-start">
                    <p class="title text-base font-bold mb-0 flex items-center   justify-center w-full  my-2 ">
