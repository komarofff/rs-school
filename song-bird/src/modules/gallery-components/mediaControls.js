...
export function mediaControls(){
    ...
    function findSelector(ell){
        ...
        if(ell.target.classList.contains('play-stop-button')){
            ...
            let playTime = selector.querySelector('.play-time')
            let mediaTime = selector.querySelector('.media-time')
            let timeLine = selector.querySelector('.time-line')
            let playStopButton = ell.target
            let volume = selector.querySelector('.volume')
            let speaker = selector.querySelector('.speaker')
            ...
            if (isPlay) {
                ...
                isPlay = true
                // currentMediaInPlayer = mediasArray[id].player
                mediasArray[id].player.play()
                playStopButton.src = "public/images/pause-button.png"

            }

            mediasArray[id].player.addEventListener('loadedmetadata', () => {
                timeLine.max = Math.round(mediasArray[id].player.duration)
                rangeMax = Math.round(mediasArray[id].player.duration)
                let calculatedTime = calculateTime(rangeMax)
                mediaTime.innerHTML = calculatedTime.minutes + ':' + calculatedTime.seconds
            })
            mediasArray[id].player.addEventListener('timeupdate', () => {
                let mediaTimeInSeconds = mediasArray[id].player.currentTime
                let calculatedTime = calculateTime(mediaTimeInSeconds)
                playTime.innerHTML = calculatedTime.minutes + ':' + calculatedTime.seconds

                if (isPlay) {
                    timeLine.value = mediasArray[id].player.currentTime
                    colorRange(timeLine)
                } else {
                    timeLine.value = mediasArray[id].player.currentTime
                    colorRange(timeLine)
            ...
            function changeTimeLineQuestion() {
                ...
                if (isPlayQuestionPlayer) {
                    //mediasArray[id].player.currentTime = timeLine.value
