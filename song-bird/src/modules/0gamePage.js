    ...
    async function startGame() {
        ...
        function goNextLevel() {
            if (nextLevelButton.classList.contains('active')) {
                ...
                if (currentIdx < tabsInCategories.length) {
        ...
        let randomElement = 0
        let birdsList = await createBirdsList()
        ...
        birdsList.arr.forEach(el => {
            if (el.name) {
                ...
                li.className = 'list-item  py-2 px-4 w-full cursor-pointer  transition duration-300 border-b border-blue-100 hover:bg-blue-500 hover:bg-opacity-75 hover:text-white relative text-sm'
        ...
        randomElement = getRandomInt(0, birdsList.arr.length - 1)
...
//плеер главного вопроса. вопрос бокс отправляем его селектор запоминаем данные вопроса для сравнения с ответами
        ...
        playerSection.innerHTML = await writeBox(null, 'player')
...
//ждем нажатия на список для сравнения
        ...
        async function createQuestionMedia(mediaUrl, selector) {
            ...
            let questionMedia = await new Audio(mediaUrl)
            console.log('questionMedia', questionMedia)
            // questionMedia.addEventListener("canplaythrough", (e) => {
            //     console.log('  аудио может быть воспроизведено; проиграть, если позволяют разрешения ', e)
            //     isAudioLoaded = true
            // })
